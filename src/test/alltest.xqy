...
{
for $test in collection("tests.xproc.org/required?*.xml")
 ...
 let $bindings  := ()
 let $options   := ()
 let $outputs   := if($alternate) then xproc:generate_output("!1","alternate","output","false",$alternate) else ()
return
...
<expected>{if ($expected) then try{u:strip-whitespace(document{$expected})} catch * {<error/>} else ()}</expected>
<result>{
let $result := try{ $xproc:run-step($pipeline,$stdin,$bindings,$options,$outputs,$dflag,$tflag)} catch * {<error type="test"/>}
return
