...
{
for $test in collection("tests.xproc.org/required?select=viewport*.xml")
...
<result>{
let $result := try{ $xproc:run-step($pipeline,$stdin,$bindings,$options,$outputs,$dflag,$tflag)} catch * {<error test="1" type="{$error}">{$outputs}</error>}
return
